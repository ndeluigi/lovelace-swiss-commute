...
class SwissPublicTransportCard extends LitElement {
  ...
  //  this.last_changed = moment.duration(moment(state.last_changed).diff(now)).humanize(true);
    ...
    for (const journey of state.attributes["departures"]) {
      ...
      // exclude if via_filter is set and journey doesn't include the intermediate stop
      if (this._config.via_filter && !this._journeyIncludesVia(journey)) {
        continue;
      }
  ...
  /**
   * Extracts stop names from a journey object.
   * Searches multiple possible data structures defensively.
   * Returns an array of stop name strings, or empty array if none found.
   */
  _extractStopNames(journey) {
    if (!journey || typeof journey !== "object") return [];

    const stops = [];

    // Add origin and destination if available
    if (journey.from) stops.push(journey.from);
    if (journey.to) stops.push(journey.to);

    // Try stopovers (common format)
    if (Array.isArray(journey.stopovers)) {
      for (const stopover of journey.stopovers) {
        if (stopover?.stop?.name) {
          stops.push(stopover.stop.name);
        } else if (typeof stopover === "string") {
          stops.push(stopover);
        }
      }
      return stops;
    ...
    // Try passList
    if (Array.isArray(journey.passList)) {
      for (const pass of journey.passList) {
        if (pass?.stop?.name) {
          stops.push(pass.stop.name);
        } else if (pass?.name) {
          stops.push(pass.name);
    ...
    // Try sections (for complex journeys with transfers)
    if (Array.isArray(journey.sections)) {
      for (const section of journey.sections) {
        if (section?.journey?.stopovers) {
          for (const stopover of section.journey.stopovers) {
            if (stopover?.stop?.name) {
              stops.push(stopover.stop.name);
            }
          }
        }
        if (section?.journey?.stops) {
          for (const stop of section.journey.stops) {
            if (stop?.name) {
              stops.push(stop.name);
            }
          }
    ...
    // Return at least origin/destination if we found them
    return stops;
  }

  /**
   * Checks if a journey includes the via_filter intermediate stop.
   * Returns true if the journey passes through the specified station.
   * Returns false if via_filter is not set, stop list is empty, or no match found.
   */
  _journeyIncludesVia(journey) {
    const viaFilter = this._config?.via_filter;
    if (!viaFilter) return true; // no filter = include all
    ...
    const fallbackInclude = this._config?.via_filter_fallback_include === true;
    ...
    const normalizedStops = caseSensitive 
      ? stops.map(s => String(s ?? "").trim())
      : stops.map(s => String(s ?? "").trim().toLowerCase());
    
    const target = caseSensitive 
      ? String(viaFilter).trim()
      : String(viaFilter).trim().toLowerCase();

    // Check if any stop matches
    if (useRegex) {
      try {
        const flags = caseSensitive ? "" : "i";
        const re = new RegExp(viaFilter, flags);
        return normalizedStops.some(stopName => re.test(stopName));
      } catch (e) {
        // Invalid regex, fall back to substring match
        return normalizedStops.some(stopName => stopName.includes(target));
      }
    } else {
      // Substring match
      return normalizedStops.some(stopName => stopName.includes(target));
